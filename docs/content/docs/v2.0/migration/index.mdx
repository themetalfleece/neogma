...
Neogma v2 is fully backwards compatible with v1. Your existing `ModelFactory` code will continue to work. The `ModelFactory` API itself is not deprecated, but the old revalidator validation schema is deprecated and will be removed in a future major release. We recommend migrating to the decorator-based API with TypeBox validation for the best experience.

## What changed in v2

- **Decorator-based models** - define models with `@Node`, `@Property`, `@Relationship` instead of `ModelFactory`
...
Key changes:
- Replace ModelFactory with @Node, @Property, @Relationship decorators
