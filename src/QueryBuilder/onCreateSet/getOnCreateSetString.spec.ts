...
describe('getOnCreateSetString', () => {
  ...
  describe('type safety', () => {
    ...
    it('rejects onCreateSet with empty string parameter', () => {
      const qb = new QueryBuilder();
      expect(() => {
        qb.onCreateSet('');
      }).toThrow("Invalid 'onCreateSet' value");
    ...
    it('rejects onCreateSet with whitespace-only string parameter', () => {
      const qb = new QueryBuilder();
      expect(() => {
        qb.onCreateSet('   ');
      }).toThrow("Invalid 'onCreateSet' value");
    ...
    it('rejects onCreateSet object with whitespace-only identifier', () => {
      const qb = new QueryBuilder();
      expect(() => {
  ...
  describe('type guard', () => {
    it('isOnCreateSetObject returns true for valid object parameter', () => {
    ...
    it('isOnCreateSetObject returns false for object with empty identifier', () => {
      const param = { identifier: '', properties: { name: 'test' } };
      expect(isOnCreateSetObject(param)).toBe(false);
    });

    it('isOnCreateSetObject returns false for object with null properties', () => {
      const param = { identifier: 'n', properties: null };
      // @ts-expect-error - testing runtime behavior with invalid input
      expect(isOnCreateSetObject(param)).toBe(false);
    });

    it('isOnCreateSetObject returns false for object missing identifier', () => {
      const param = { properties: { name: 'test' } };
      // @ts-expect-error - testing runtime behavior with invalid input
      expect(isOnCreateSetObject(param)).toBe(false);
    });

    it('isOnCreateSetObject returns false for object missing properties', () => {
      const param = { identifier: 'n' };
      // @ts-expect-error - testing runtime behavior with invalid input
      expect(isOnCreateSetObject(param)).toBe(false);
    });

    it('isOnCreateSetObject returns false for object with whitespace-only identifier', () => {
      const param = { identifier: '   ', properties: { name: 'test' } };
      expect(isOnCreateSetObject(param)).toBe(false);
    });
  });
