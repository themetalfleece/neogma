...
export const ModelFactory = <
    ...
    const { primaryKeyField, relationshipCreationKeys, schema } = params;
    const label = QueryRunner.getNormalizedLabels(params.label);
    ...
    class Model {
        ...
        private async createFromInstance(data: CreateDataI | Instance, session: Session) {
            ...
            const objectsCreateRes = await queryRunner.createMany(session, label, [dataToCreate], 'nodes');
        ...
        public static async createMany(
            ...
            return getSession(configuration.session, async (session) => {
                if (!relationships.length) {
                    ...
                    const res = await queryRunner.createMany(session, label, instances, 'nodes');
