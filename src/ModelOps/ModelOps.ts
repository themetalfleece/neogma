import { QueryResult, Session, session } from 'neo4j-driver';
...
export const ModelFactory = <
    ...
    class Model {
        ...
        private async createFromInstance(data: CreateDataI | Instance, session: Session) {
            ...
            const objectsCreateRes = await queryRunner.createMany(
                session,
        ...
        public static async createMany(
            ...
            return getSession(configuration?.session, async (session) => {
                if (!relationships.length) {
                    ...
                    const res = await queryRunner.createMany(
                        session,
                        QueryRunner.getNormalizedLabels(modelLabel),
                        instances,
                        'nodes'
        ...
        public static async editOne(
            ...
            return getSession(configuration?.session, async (session) => {
                const res = await queryRunner.editMany(session, normalizedLabel, data, where);
                return getResultArrayFromEdit<Attributes>(res, normalizedLabel)[0];
            });
        }
        ...
        public static async editMany(
            ...
            return getSession(configuration?.session, async (session) => {
                const res = await queryRunner.editMany(session, normalizedLabel, data, where);
                return getResultArrayFromEdit<Attributes>(res, normalizedLabel);
            });
        }
